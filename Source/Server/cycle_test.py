...
def dir_copy():
    root_src_dir = r'/tmp'
    root_dst_dir = r"/home/usergpu2/kbbank_label"
    ...
    for src_dir, dirs, files in os.walk(root_src_dir):
        ...
        for file_ in files:
            ...
            if file_ == 'output_graph.pb':
                src_file = os.path.join(src_dir, file_)
                dst_file = os.path.join(dst_dir, file_)
                if os.path.exists(dst_file):
                    os.remove(dst_file)
                shutil.move(src_file, dst_dir)
            if file_ == 'URLlist.txt':
                src_file = os.path.join(src_dir, file_)
                dst_file = os.path.join(dst_dir, file_)
                if os.path.exists(dst_file):
                    os.remove(dst_file)
...
def inception_retrain():
    ...
    def main(_):
        ...
        FLAGS.image_dir = r"/home/usergpu2/kbbank_capture"
...
def capture_website():
    ...
    def url_save(url):  # URL을 텍스트 파일에 저장하는 함수
        f = open(r"/tmp/URLlist.txt", 'a')
        f.write(url+"\n")
        #f.write("\n")
        f.close()
...
def execute_func(second):
    global end
    if end:
        return
    f = open(r"/tmp/URLlist.txt", 'w')
    f.close()
    capture_website()
    sleep(3)
    inception_retrain()
